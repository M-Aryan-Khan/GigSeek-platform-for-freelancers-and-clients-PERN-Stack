...
export default function ClientPastOrders() {
  ...
  const fetchPastOrders = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:5000/gig/client/orders/history",
        {
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-6 mt-8 max-w-7xl">
        ...
        <button className="px-4 py-2 hover:bg-[#0187a4] hover:text-white rounded bg-gray-100 text-[#0187a4]" 
          ...
          <button
            className="px-4 py-2 bg-[#0187a4] text-white rounded"
          >
            History
          </button>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {pastOrders.map((order) => (
              ...
              >
                ...
                <p className="text-gray-600 mb-1">Owner: {order.freelancer_name}</p>
