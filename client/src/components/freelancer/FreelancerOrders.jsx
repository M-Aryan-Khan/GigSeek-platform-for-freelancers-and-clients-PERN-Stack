  ...
  const fetchPendingOrders = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:5000/gig/freelancer/orders/pending",
        {
    ...
    <>
      ...
      <div className="container mx-auto px-4 py-6 mt-8 max-w-7xl">
        ...
        <div className="flex gap-4 mb-6">
          <button className="px-4 py-2 bg-[#0187a4] text-white rounded hover:bg-gray-100 hover:text-[#0187a4]">
            Pending Orders
          </button>
          <button
            onClick={() => navigate("/freelancer/orders-history")}
            className="px-4 py-2 bg-gray-100 text-[#0187a4] rounded hover:bg-[#0187a4] hover:text-white"
          >
            History
          </button>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {pendingOrders.map((order) => (
              ...
              >
                ...
                <div className="mt-4">
                  <Link to={`/freelancer/order/${order.order_id}`}>
